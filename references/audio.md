# 人声、背景音乐、音效与最终混音

## 两阶段音频流程

音频拆成两个阶段：

1. `dialogue_preprocess`：先分离人声与非人声，再只处理通过验收的 dialogue stem；
2. `final_mix`：视觉时序冻结后再加入 BGM、SFX、闪避和母带。

这样可以避免转场、文字动画和插镜时序变化后重复整链处理人声。

## 人声分离：必须先于降噪和增强

原始音轨、相机机内声、来源不明的导出音轨或已经混有环境声/BGM 的音轨，默认先生成分离候选：

```bash
scripts/separate_dialogue.sh INPUT.wav SEPARATION_DIR
```

Demucs model、device、时长容差和可选二进制路径默认从咔嚓配置读取；当前命令
的显式参数与 `KACHA_DEMUCS_BIN` 优先。

标准产物：

- `original_reference.wav`：对齐后的原始参考，只用于同响度 A/B 和回退；
- `dialogue_isolated.wav`：候选人声；
- `non_dialogue_residual.wav`：环境声、原有音乐及其他非人声候选残余；
- `separation-report.json`：引擎、模型、源文件哈希、时长/采样率/声道和验收状态。

这不是可选的“增强插件”。只要项目包含口播并进入音频处理，`sourceSeparation.required` 就必须为 `true`。最终 dialogue stem 的合同是“只保留验收通过的人声内容”：讲话、语义相关的自然换气、笑声和叹气可以保留；风扇、空调、交通、碰撞、房间底噪、原有音乐和其他无关声音必须从 dialogue stem 中剔除。

处理顺序固定为：

1. 先删除重说、口误、废句、异常噪声段和无意义停顿，冻结粗剪；再从同一组
   音画边界导出待处理音轨，真实转场边界使用与画面等长的短 crossfade；
2. 能力探测确认真实源分离引擎可用；
3. 生成 `dialogue_isolated` 和 `non_dialogue_residual`；
4. 对原始参考与独立人声做同响度 A/B；
5. 检查 residual 中是否仍有清晰语音泄漏；
6. 通过后只把 `dialogue_isolated` 送入降噪、人声增强和最终混音，`non_dialogue_residual` 不混回成片；
7. 后期需要的 BGM 和 SFX 使用独立新 stem 重新加入。

`separation-report.json` 中 `acceptance.approved=false` 或
`status=candidate_requires_ab` 表示“待听审候选”，不能被最终时间线标为
已验收 dialogue。自动频谱、时长和 residual 检查不能替代同响度 A/B；未完成
听审时只能交付返修候选并明确保留风险。

“人声”包括实际讲话、语义相关的自然换气和有表达作用的笑声/叹气；风扇、空调、交通、房间底噪、原有 BGM、碰撞声和无关环境声属于待剔除内容。口水音、爆音和齿音属于人声局部缺陷，在分离后单独修复，不能依赖源分离模型误删。

源分离是有损推断，不因用户要求“剔除非人声”就跳过验收。以下任一情况直接拒绝候选，并阻止“只保留人声”的后续链路：

- 辅音、尾音、轻声或吞音明显缺失；
- 齿音发虚、声音呈金属/水下质感；
- 呼吸、停顿或混响出现抽吸和门限开合；
- 声纹、口型同步、声道或时长被破坏；
- residual 中仍存在可辨认的完整词句；
- 只使用 `dialoguenhance`、中心声道提取、EQ 或普通降噪，却把结果标为人声分离。

保留 residual 是为了复核和回退，不表示允许把它重新混回最终成片。确需保留现场氛围时，应从许可明确、没有语音泄漏的独立 ambience stem 重新建立，并在方案中说明；不得直接复用含噪 residual。

如果源分离候选损伤人声，项目必须标记 `blocked`。只有用户明确授权降低目标后，才能另建“原始参考降噪”方案；该方案不得标记为已完成人声分离。任何情况下都不能一边声称“只保留人声”，一边悄悄把 residual 混回去补质感。

## 缓存、转写与资源合同

人声链的高成本步骤不得在同一源、同一参数和同一实现上重复运行：

- `separate_dialogue.sh` 以源 SHA、Demucs model/device、Demucs 与 Torch
  运行时版本、启动器/入口模块实现 SHA 和输出 schema 复用
  `source_separation` 缓存；运行时变化必须使旧结果失效；
- `kacha transcribe INPUT --output transcript.json` 默认调用配置中的本机
  loopback Whisper MLX 服务；视频输入先按 `audioStreamIndex` 选择真实音轨，
  规范成 16 kHz 单声道 PCM 再提交识别，避免把 4K 多音轨容器直接送入服务。
  `conditionOnPreviousText` 默认关闭，防止长停顿后的重复幻觉；有原稿时通过
  `--prompt` 提供术语上下文，而不是让错误文本跨窗口自我强化。模型、语言、
  音轨、规范化参数和实现 SHA 全部进入 ASR 缓存指纹；
- 完整转写和逐词时间戳留在文件中；agent 先用
  `transcript index`，再按最多 180 秒的 `transcript slice` 读取；
- CPU Demucs 占用 `cpuHeavy`，MPS Demucs 占用 `mps`。同一时刻只允许一个
  重型 MPS 任务，避免与 Beauty、Vision 或视频编码争抢内存；
- 所有真实执行使用 `metrics run` 记录墙钟时间、缓存命中、Token、日志和
  产物。完整日志写文件并脱敏，不把模型或 FFmpeg 大日志塞回 agent 上下文。

缓存命中仍校验输出文件、目录清单与 SHA-256。分离或 ASR 的模型、实现、
参数、源内容变化时必须失效；不能为了提速复用旧证据。

## 人声诊断

先提取并检查：

- 持续讲话；
- 静音或弱声；
- 齿音、送气音；
- 爆音、口水音；
- 轻声与结尾；
- 有 BGM 的参考段。

记录噪声底、频谱、综合响度、true peak、声道结构、采样率和编码。只凭全片 LUFS 不能决定是否降噪。

## 人声增强

```bash
scripts/enhance_voice.sh INPUT.wav OUTPUT.wav \
  --preset warm-soft \
  --denoise light \
  --target-lufs -21 \
  --true-peak -4 \
  --channel-mode preserve
```

preset、降噪、declick、目标响度、true peak 和声道策略均可放入
`execution.voiceEnhancement`；命令行参数用于当前项目覆盖。

默认 `warm-soft + light denoise`，面向需要长时间聆听的知识口播；但 A/B
出现金属声、抽吸声、辅音损失、齿音变形或呼吸断裂时必须减弱或关闭。

- `natural`：保留本人声纹；
- `warm`：轻微增加厚度；
- `warm-soft`：默认长听感预设。160 Hz 低架 `+1.4 dB`，280 Hz 宽峰
  `+2.0 dB`，850 Hz `-1.5 dB`，2.5 kHz `-2.0 dB`，3.8 kHz
  `-1.5 dB`，6.5 kHz 高架 `-1.0 dB`；配合轻度 de-esser 和约
  `1.5:1` 的柔和瞬态控制；
- `clear`：同响度下减少低中频堆积并克制提升存在感，不等于简单抬高高频；
- `off/light/medium`：降噪强度；
- `arnndn` 只有提供来源可信、许可明确且经过 A/B 的模型时使用。

脚本默认保留源声道，不再无条件压成 mono 后复制为伪立体声。只有明确的 dialogue stem 交付要求才使用 `--channel-mode mono|stereo`。不得把完整 voice/BGM/SFX 母带交给人声增强脚本。

口水音和爆音优先局部修复，不对全片强行 `declick`。本地链路可向成熟产品的自然听感靠近，但不声称与未公开商业模型一键等价，也不承诺恢复原录音不存在的细节。

## 同响度 A/B

参考另一版本时先分离：

- 综合响度；
- 频谱平衡；
- 动态与峰均比；
- 声道结构；
- 采样率和编码；
- BGM 掩蔽关系。

候选和参考使用同源时间段，并把响度匹配到约 `±0.2 LU`。更响、单声道化或低码率柔化都不能被误判为更清澈。

已经降噪、去齿音或压缩的人声默认不重复整链处理。优先回到处理前 stem；如果只能使用已处理 stem，只修改有证据的问题节点。

## 人声动态与响度

压缩只修复技术问题，不能抹平自嘲、结论、章节转换、停顿后的落点和轻声收尾。

知识类成片可从以下范围开始校准：

- 常规版本：约 `-18 LUFS`，true peak 不高于 `-2 dBTP`；
- 用户要求平台适配后再低一点：约 `-20` 至 `-21 LUFS`，true peak 约 `-3` 至 `-5 dBTP`。

这些不是平台统一标准，最终使用手机扬声器和耳机实听。

默认“更暖更柔”长听感混音以 `-21 LUFS`、true peak 不高于
`-4 dBTP`、最终 LRA 约 `4.5–5.5 LU` 为起点。连续口播中的 BGM 通常保持
在人声下约 `18 dB`，宽度约 `0.5`；SFX 通常在人声下约 `12 dB`，并从
`4.5 kHz` 起轻收约 `1.5 dB`。这些是最终关系，不是把任意原始素材机械执行
固定增益；每次仍需按 stem 实测和同响度 A/B。

统一时间线的最终混音必须在 `audio.masterTruePeakDb` 显式声明母带峰值上限，
默认沿用 `-4 dBTP`。渲染器把 dBTP 合同换算为线性 limiter 上限；不得在各
stem 已按目标处理后又用接近 `0 dBFS` 的硬编码总线限制器抬高峰值。最终仍以
解码后的成片实测 true peak 为准，而不是以滤镜参数代替验收。
FFmpeg `alimiter` 必须显式设置 `level=false`，否则默认自动补偿会把限制后的
信号重新抬高。QC 重建 component stems 时必须使用与时间线相同的 limiter
上限和 `level=false`，否则会把正确母带误判为 stem 不一致。

## 自然口播参考基准（中文版标准）

当用户指定一个已认可版本作为人声参考时，参考的是自然度、动态和听感，不是
复制它的编码参数。默认执行以下合同：

- 候选与参考必须取同类语气、同源或语义相近片段，先匹配到约 `±0.2 LU`，
  再比较音色；禁止把更响误判为更清澈；
- 单人固定机位口播在原始 dialogue 明显偏离中心、且没有有效空间叙事时，
  可以采用 `centered_dialogue`：先以等权下混生成单声道人声，再复制为居中
  双声道。执行前后都要检查相位、单声道兼容和齿音，不能对音乐母带这样做；
- 当前“中文版自然口播”参考区间是：dialogue LRA 约 `4.5–6 LU`，最终成片
  LRA 约 `4–6 LU`，综合响度约 `-20` 至 `-21 LUFS`，true peak 约
  `-3` 至 `-5 dBTP`。这些是起始区间，不得为了命中数值制造抽吸、底噪起伏
  或不自然的字头；
- 英文口播通常要额外复核 `1.5–4 kHz` 的硬度和 `6–10 kHz` 的齿音，但语言
  音系不同，不能机械复制中文版 EQ 数值；以同响度的人声质感、辅音清晰度和
  长时间不疲劳为准；
- 动态修复完成后优先使用常量增益对齐目标响度，只在峰值越界时使用峰值限制。
  禁止在最终母带末端使用会显著压低 LRA 的整段动态 `loudnorm`；
- 已经降噪、去齿音或压缩的 stem 不重复整链处理。需要恢复语气起伏时回到
  最早可用的 dialogue stem，只做有证据的温和扩展、EQ 和峰值控制；
- 只要求优化人声时，冻结视频、字幕、BGM、SFX、sidechain 和事件时序。
  通过视频 elementary-stream SHA-256 证明画面未变；
- 源分离报告必须为已批准状态。`approved: false` 的候选不能被写成完成了
  人声分离；如果只能沿用已处理的人声，须在交付记录中如实标明继承边界；
- 最终至少保留一份同响度 A/B，覆盖普通叙述、强调句和轻声收尾，并在手机
  扬声器与耳机上确认：主体居中、齿音不过亮、口水音不过分突出、强弱仍自然。

## 自适应 BGM

BGM 应能感知但不抢注意力，也不能从头到尾保持同一条循环、同一编配和同一
响度。先按最终对白建立 `adaptive-bgm-plan.json`：逐段判断叙事功能、情绪、
语速、信息密度和环境声价值，再决定音乐进入、变奏、退出或保持留白。

```bash
node scripts/kacha.mjs bgm plan \
  --cues semantic-cues.json --show tool-share \
  --style xingzhe --output adaptive-bgm-plan.json
node scripts/kacha.mjs bgm validate --plan adaptive-bgm-plan.json
```

### 专业提示词合同

每个有音乐的段落必须明确 `purpose / duration / bpm / meter / groove /
instruments / timbre / harmony / frequencyStrategy / dynamics / stereo /
editPoints / negativePrompt`。提示词不能只写“高级、温暖、有氛围”：

- 乐器与节奏：按说话速度选择脉冲密度；快速说明减少音符，操作步骤才增加
  短促节拍，反思段移除打击乐；
- 音色与和声：全片保持共同的二至四音动机、调性中心和一组核心音色，优先
  改变编配密度、音区与和声色彩，不把动态理解成频繁换歌；
- 低频：约 `45–60 Hz` 以下谨慎高通，`70–180 Hz` 低频短而克制，避免与
  爆破音、胸腔共鸣和环境底噪叠加；
- 人声频段：`180 Hz–4 kHz` 尤其 `1–3.5 kHz` 给旁白留中心空间，pad 与
  主旋律避开人声共振和语音中心；
- 高频：持续的 `6–10 kHz` 镲片、shaker 和空气感要收敛，不能盖住辅音；
- 声像：低频保持 mono，纹理与氛围可放两侧，中间稳定留给对白；
- 动态：入场、退场和变奏同时服从语义边界与四/八小节乐句，生成素材保留
  可用 fade handle 和干净尾音；禁止 audible pumping；
- 负面约束：固定排除歌词/人声、占据人声区的强主旋律、繁忙琶音、低频轰鸣、
  高频常亮、预告片式冲击、廉价企业配乐和游戏循环感。

栏目级语法由 `config/audio/adaptive-bgm-policy.json` 统一管理：

- 工具分享：`92–112 BPM`，短促电子拨弦、干燥打击与轻 marimba 服务步骤和
  结果；参数、数字密集处退乐；
- 解读好书：`60–82 BPM`，felt piano、木质/室内乐纹理；核心论证和引用大多
  保持纯人声，音乐主要用于开场、章节、反思和结尾；
- 有限的无限游戏：`68–96 BPM`，地点环境声优先，使用有机打击和真实材质；
  只在过程、时间跳转和情绪转折进入，禁止英雄化；
- 灰常AI：`96–124 BPM`，干燥 modular pluck、克制 glitch 与短模拟和弦；
  允许喜剧停顿，事实核查区间必须收稳或退出。
- 闲聊：`72–96 BPM`，以温暖短拨弦、轻刷鼓、克制木质打击和少量空气纹理
  服务语气与话题换挡；音乐覆盖不超过 52%，单次连续不超过 36 秒，观点密集、
  轻声、停顿、笑点和开放问题优先退乐或留白。

### 混音与执行合同

- 一般段落 BGM 约在人声下 `18 dB`；hook/结果可约 `14 dB`；高密度、事实、
  轻声和专有名词段约 `21 dB` 或直接退出，合法范围 `12–24 dB`；这些数值
  是起点，最终以频谱、表演和设备实听为准；
- sidechain 以约 `18 ms attack / 320 ms release / 最大约 6 dB gain
  reduction` 起步，只恢复可懂度，不制造抽吸；
- Timeline 使用 `audio.bgm.segments[]` 绑定每段真实 WAV、时间、源入点、相对
  人声音量、fade、SHA-256 和 provenance；渲染器会把段落、留白与分段增益
  合成为 BGM stem，再统一做人声闪避；
- 项目设置 `expectedMedia.audioMix.adaptiveBgmRequired=true`，并让
  `plans.adaptiveBgm` 与 `audio.bgm.adaptivePlan` 绑定同一文件及真实 SHA；
- 最终混音必须导出独立 dialogue/BGM/SFX 组件 stem 与 mix stem，并测量
  **闪避之后**的综合响度；不能只检查 BGM 源文件存在、混音命令执行成功或
  最终母带总响度；
- 项目要求 BGM 时，在 `outputs.audioStems` 声明组件和 mix stem，并在
  `expectedMedia.audioMix` 设置 `bgmRequired=true`。`qc` 会检查 BGM 时长
  覆盖和人声/BGM 实际差值、重建 mix，并将最终视频解码音频与 mix stem
  比对；差值高于上限或成片漏混都直接失败；
- 增量返工创建 manifest 时传入 `--dialogue-stem <post-mix.wav>` 与
  `--bgm-stem <post-sidechain.wav>`、`--mix-stem <final-mix.wav>`（有 SFX
  时再传 `--sfx-stem`），沿用同一门禁，不能因“只改音频”而绕过；
- 用户说“再低一点”时，以认可版本为基线，每轮只降低 BGM 约 0.5–1.0 dB；
- 同时冻结人声、SFX、sidechain 和母带；
- 禁止降低整片音量冒充 BGM 变小；
- 入场、章节换挡和结尾服从音乐乐句，不为对拍切断语义。

QC 对自适应项目只在计划中的音乐/对白重叠区间计算相对响度，避免把有意留白
误判成“音乐太小”；同时仍要求 BGM stem 覆盖完整时间轴、留白真实存在、组件
可重建 mix、最终视频音轨与 mix stem 匹配。正常速度人工听审至少覆盖开场、
密集事实、轻声、一次音乐退出、一次编配变化、结论和结尾，并使用监听、耳机与
手机扬声器三种环境。

## SFX

音效只强化已命名的动作、文字落位、转场或叙事落点：

- 用户点名本地音效时，读取 `references/sfx-library.md`，按 title、asset ID 和 SHA-256 精确命中；
- 打字效果的每个键音绑定字符实际落位帧，不能把一段连续键盘声铺在整句话下；
- “相似”“差不多”的音效不是可接受替代；精确素材不可用时宁可不加并报告；

- 低于人声，但最终成片中必须真实可感知；
- 与动作/文字误差通常不超过 1–2 帧；
- 必要时短暂闪避 BGM；
- 每个语义节拍通常只有一个主要声音事件；
- 不连续轰炸，不用大音量制造重要感。

整支视频不能把“统一风格”误解为“所有地方使用同一个音效”。冻结视觉时序后建立顶层 `sfxPlan`：

- `palette` 按功能至少区分文字/UI 落位、转场/运动、结论/情绪等类别；
- `events` 逐项记录时间、效果引用、asset ID、title、category、purpose、同步目标和相对人声音量；
- 4 个及以上事件至少使用 3 个不同音效，单个音效占比不超过 50%；
- 同一音效不得连续出现超过两次；逐字打字、连续列表等同一动作的节奏例外必须记录 `patternException`；
- 同一声音可以通过裁切不同段、轻微增益和空间处理形成变化，但不能把同一源文件重命名后冒充不同音效；
- 多样性服从功能，不为凑数量乱加声音；没有合适音效时保持安静。

最终 QC 同时检查“是否听得见”和“是否听起来都一样”。若多个转场、弹窗、文字落位使用同一音色且没有动作理由，退回重新选音。

## 最终混音 QC

至少保留或临时导出 `voice`、`bgm`、`sfx` 三类 stem，并检查：

- `original_reference / dialogue_isolated / non_dialogue_residual` 齐全，分离引擎和 A/B 结论可追溯；
- 最终 `voice` 来自已批准的 `dialogue_isolated`，非人声 residual 没有进入母带；
- residual 抽听无可辨认完整词句，人声 stem 无吞字、金属声、抽吸或呼吸断裂；
- 各 stem 响度和 true peak；
- sidechain 是否过度压制；
- BGM 进入、退出和章节换挡；
- 每个 SFX 落点；
- 开头、持续口播、轻声、章节转换和结尾；
- 手机扬声器与耳机。

最终视频保持 48 kHz 音频链路，不能无理由降到 44.1 kHz 或降低 AAC 码率。

## MiniMax 音乐网络策略

需要 MiniMax 音乐时默认中国区无代理直连：

- 清除大小写 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY`；
- 设置 `NO_PROXY=*`；
- 授权、额度、生成和下载使用同一路由；
- 先预检，再发起付费调用；
- 提交后网络失败先查任务状态，状态不明不得自动重提；
- `music-2.6` 可请求 44.1 kHz WAV，下载后本地转为 48 kHz；
- 记录模型、参数、prompt 哈希、路由、任务 ID、调用次数和计费不确定性。
