...
流程、阶段和清单类内容默认从五套独立视觉语言中选择，不再把一个视觉样式硬套到
所有内容：
...
- `xingzhe-pixel-editorial`：像素风。以 6–12 px 基础网格、有限强调色、硬边图形
  ...
  复盘，不像素化人物、事实证据和可读文字。
- `xingzhe-dark-tech`：暗黑科技风。只在异常、风险、冲突证据、隐性机制、真伪
  核验或系统边界中使用；局部隔离异常、锁定证据并落一次裁决，禁止整屏黑化、
  通用赛博 HUD、连续扫描和随机 glitch。

五者都必须解析对应 `motionContract`。合同不是参考文案，而是正式执行输入，
至少包含：
...
以上五套风格不得使用其他字体。任一字体缺失、SHA 改变或缺字时阻断；确需例外时必须记录原因并通过可见预检，不静默回退。漫画、像素和暗黑材质只能作用于图形层，不能把字幕替换成漫画字、点阵字或 HUD 字。
