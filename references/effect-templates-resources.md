...
- `设计参考/行者风高保真样板/全量效果库_行者风3_浅暖轻浮层/`
- `设计参考/行者风高保真样板/全量效果库_行者风3_空间光路/`
- `设计参考/行者风高保真样板/全量效果库_行者风3_幽默漫画/`
- `设计参考/行者风高保真样板/全量效果库_行者风3_像素风/`
- `设计参考/行者风高保真样板/全量效果库_行者风3_暗黑科技风/`
...
构图、头脸覆盖、空间近黑块、未允许精确重复、旧版产物和清单外孤儿图均为 0；
每个 manifest 内嵌合同还必须与权威注册表逐字段完全一致。后续任何一项回归都会使
...
- `effect-process_light_overlay`：浅暖轻浮层。实拍背景完整可见，以无边界编辑排版、
  边缘旁注和最多一个必要的半透明局部表面建立层次；当前项局部高亮，禁止小白卡
  阵列；适合知识、方法、清单和更亲和的叙述。
...
1. 原镜头重构：景别、构图、关键帧与节奏性推拉；
2. 真实证据插镜：实拍、屏录、原书、项目文件与来源；
3. 空间分层：人物蒙版、前景遮挡、焦点转移与有信息差的画中画；
4. 无边界排版：边缘旁注、关系字幕、人物后短字与空间标注；
5. 全屏证据、分屏对比、反应插格或过程路径；
6. 只在信息必须保持边界时选择有边界表面。

用户明确指定的效果仍需经过语义触发、安全区和 `anti-web.json` 预算检查。
没有叙事理由时保持原镜头；存在更简单的电影化方案时，不得直接跳到卡片或弹窗。
