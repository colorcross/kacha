...
默认 `config/capability-usage/xingzhe.json` 使用行者风 3.0 的栏目级
`showProfiles`。
...
能力覆盖不得通过增加卡片和弹窗凑数。每个项目还必须执行
`config/design-system/anti-web.json` 的栏目画面预算，先计算真人/证据占比、
有边界表面占比、仪表盘占比和镜头机制多样性，再验证能力数量。
