...
1. **素材**：选择或粘贴视频绝对路径，读取尺寸、宽高比、帧率、时长和
   音视频编码；
2. **风格**：先选择 `行者风`、`清朗知识`、`沉浸阅读`、`现代科技`、
   `沉静纪录`等基础品牌预设，再选择“自动按语义”或优先使用
   `浅暖轻浮层`、`空间光路`、`幽默漫画`、`像素风`中的一套剪辑视觉语言；
   也可基于现有基础风格另存本地自定义风格；
...
画面条件在四套语言中选择，没有匹配信号时保持干净画面或普通字幕。选择某一套
...
`config/production-studio.json` 保存默认规则和基础风格模板；四套剪辑语言的
