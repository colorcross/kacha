...
```text
Install the latest Kacha skill from https://github.com/colorcross/kacha.git. Detect whether you are Codex or Claude Code, inspect and run scripts/install.sh for the matching user-level skills directory, and do not overwrite an existing installation or upload any local files. If the target already exists, report it without changing it. Run the secret scan and regression tests, then immediately read the installed SKILL.md and required references so it is usable in this session. Report the install path, version, and verification results.
```
...
The installer only needs `curl`, `tar`, and Python 3. The default locations are `~/.agents/skills/kacha-kacha` for Codex and `~/.claude/skills/kacha-kacha` for Claude Code. Node.js 20+ is required to run the gates. FFmpeg and `jq` are required for the full core workflow. Apple Vision mask generation is macOS-only.
