...
咔嚓咔嚓遵循 Agent Skills 的 `SKILL.md` 结构，同时支持 Codex 和 Claude Code。
...
| Agent | 用户级安装目录 | 当前会话 |
| --- | --- | --- |
| Codex | `~/.agents/skills/kacha-kacha` | 安装后让 Codex立即读取 `SKILL.md`；后续会话自动发现 |
| Claude Code | `~/.claude/skills/kacha-kacha` | 已存在 skills 顶层目录时支持热发现；首次创建顶层目录可能需要重启 |
...
```text
用咔嚓咔嚓先检查这批素材，给我一份详细剪辑方案，先不要改文件。
```
