...
# 咔嚓 vendored 补丁：手部素材在仓库 assets/whiteboard/ 下，可用环境变量覆盖。
DEFAULT_HAND = Path(os.environ.get(
    "KACHA_WHITEBOARD_HAND",
    str(_SCRIPT_DIR.parent.parent / "assets" / "whiteboard" / "drawing-hand.png"),
))
...
class RegionStreamRenderer:
    ...
    def __init__(self, image_bgr: np.ndarray, annotation: dict, cfg: sr.Config,
        ...
        if not bare_tip:
            # 咔嚓补丁：target_hand_height 按 1080p 调校，小尺寸输出（预览、
            # 测试）必须等比缩放，否则手部素材会盖住大半画面。
            hand_height = max(24, round(cfg.target_hand_height * self.out_h / 1080))
            ...
            for idx, element in enumerate(elements):
                ...
                if cfg.ink_path_mode == "skeleton":
                    ...
                    if path:
                        ...
                        # 咔嚓补丁：上游此处多传了一个 None 实参，空墨迹区域
                        # （区域在空白纸面或被完全遮蔽）必然 TypeError 崩溃。
                        print(f"  [warn] 区域 {element.get('id', '?')} 的可作范围内没有墨迹，跳过该区域的笔迹揭示")
