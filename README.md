...
| 语义审片台 | 1× 正常速度查看高影响决定；每项预览和解决证据必须是可解码、有视频、音轨和代表时长的真实媒体，scope 由当前项目证据确定 |
...
| NLE 语义交换 | OTIO/FCPXML 保留已知语义 ID 并绑定基线/源片 SHA，分数帧率用标准有理数；导入只生成不可覆盖的候选时间线 |
...
当前仓库完整回归为 127 项；四套图库另有跨风格重复、同风格未声明近似构图、
