...
| 语义审片台 | 1× 正常速度查看高影响决定；每项预览必须可解码、有视频、音轨和代表时长，逐项接受、调整或拒绝 |
| 可解释偏好学习 | 只从明确审片结果重建候选；按 scope 合并，需显式激活，可版本回滚，不保存自由文本内容 |
| 真实编辑评测 | 测量首稿可用、语义损坏、人工干预、连接、字幕和风格违规；8 个同源项目且关键护栏无退化才允许整体提升声明 |
| NLE 语义交换 | OTIO/FCPXML 保留语义 ID 并绑定基线/源片 SHA，分数帧率用标准有理数；导入只生成候选时间线 |
...
| 效果参考图库 | 240 个注册效果均有浅暖轻浮层、空间光路、幽默漫画与像素风四套横竖参考图，并绑定 960 份可执行动效合同 |
| 四种剪辑语法 | 连续编辑旁注、单次空间导航、喜剧节拍与确定性状态机分别组织镜头、空间、转场和声音；七轴门禁阻止只换材质的“换皮” |
...
当前版本包含 52 个设计组件、69 个复用场景、62 个预制效果模板、22 个公共
核心资源、10 种转场、5 种开场、5 种画面呼吸运动、7 种口播字幕布局，以及
从 6 条参考视频中验证出的 33 种语义网感机制。机制可从最终带时间文稿生成
帧级计划、进入完整视频渲染，并通过摘要、资源、时序与媒体保真门禁。
当前仓库完整回归为 126 项；四套图库另有跨风格重复、同风格未声明近似构图、
人物头部碰撞、黑块、字体和动效合同专项 QC。当前提交的四库报告中上述问题均为 0。

<p align="center">
  <a href="docs/FOUR_STYLE_EDITING_GRAMMARS.md">
    <img src="assets/screenshots/kacha-four-editing-grammars.png" alt="浅暖轻浮层、空间光路、幽默漫画与像素风的四种独立剪辑语法" width="100%">
  </a>
</p>
