...
test("server-renders the Chinese Kacha product page", async () => {
  ...
  assert.match(html, /1200 组语义三元组/);
  assert.match(html, /近似构图 0/);
  assert.match(html, /跨风格完全重复 0/);
...
test("server-renders the English product page", async () => {
  ...
  assert.match(html, /1,200 semantic triads/);
  assert.match(html, /0 near duplicates/);
  assert.match(html, /0 cross-style exact duplicates/);
