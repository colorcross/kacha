...
- 深度复核 OpenMontage 启发的生产控制面：付费预占改为一次性原子消费并绑定执行
  ...
  补齐结构、命令、决策和本地只读边界。
- 双 Agent skill 同步不再打包仓库级 AppCreate `quality/` 与 `tools/` 控制面，
  防止项目审查记录和依赖外部 AppCreate 包的检查器污染用户级运行安装。
- 本地生产台补齐行者风四套剪辑视觉语言入口：基础风格与剪辑语法分层展示，
  支持自动按语义或指定语言优先；选择结果进入 brief、项目配置、Agent 指令和
  配置摘要，未知语言、无触发强套风格和仅在页面显示但不落盘均被阻断。
- 深审并加固 V8 效率层：修复全范围变化零预览、离散区间机械长分组、重算摘要
  绕过预算、旧 cues/delta 静默丢失、执行器未绑定命令输出与任务后符号链接、缓存
  内容键/路径证据不足、任意旧缓存误计预热，以及格式正确但无当前文件支持的效率
  声明；项目状态和观察层改为实时重验，损坏计划可阻断并从当前输入恢复。
...
- 增加版权感知的参考片分析与原创派生、片段级素材 corpus 与 MMR 多样性检索、
  Series/Hero 制作模式决策和只读制作飞行记录；无向量证据时明确标记关键词回退，
  Studio 只读展示项目事件、遥测、任务、费用和能力决策。
- 增加参考片原创化、长视频切片、真实屏幕演示和本地化配音四套可校验工作流包；
  工作流包只生成安全引用的既有 Kacha 命令清单，不建立第二套编排状态机。
- 增加第五个正式栏目“闲聊”（`casual-chat`）：生产台、设计模式、可感知能力
  配额、自适应 BGM、评测 cohort、封面与 IP 合同统一落盘；以具体触发、问题
  拆分、暂定判断和主动留白区别于工具、读书、纪录与 AI 实验栏目。
- 增加自适应 BGM 生产链：五栏目音乐语法从语义 cues 生成音乐/留白段落与
  专业提示词，Timeline 支持多段音乐、分段增益、fade 和统一 sidechain；计划
  身份、真实素材、计划区间相对响度、stems、组件重建与最终成片漏混进入门禁。
- 增加 V8 质量不降级效率层：从当前 cues 或 version delta 生成实际代表区间，
  按 prerequisites、资源容量和输出组编排安全波次；本地自动任务同时经过主机
  资源锁与遥测，高成本缓存新增源/实现/输出强指纹审计。
- 增加效率证据门禁：只有至少 8 个同源成对、真人审片项目在语义、连接、字幕、
  视觉、声音和完整通看护栏均无退化且实测墙钟时间改善时，才支持效率提升声明。
